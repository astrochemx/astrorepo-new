...
const [parserMDX, pluginMDX] = await Promise.all([
  loadPlugin<typeof import('eslint-mdx')>('eslint-mdx'),
  loadPlugin<typeof import('eslint-plugin-mdx')>('eslint-plugin-mdx'),
] as const);
...
export async function mdx(): Promise<FlatConfigItem[]> {
  ...
  const filesAll = [...new Set([...filesBlocksInMDX, ...filesMDX])];
