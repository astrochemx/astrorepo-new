...
_precheck() {
  if ! which sg_map &>/dev/null; then
    echo "install scsi utils"
...
_scsi() {
  ...
  if [[ -z $USB_ID ]]; then
    echo "USB_ID of iflink '$DEV' not found, check arguement"
    return 2
  fi
  ...
  for i in $(seq 30); do
    if [[ -e /sys/bus/usb/devices/$USB_ID/$USB_ID:1.0 ]] &&
      ...
      grep -q icFFiscFFipFF /sys/bus/usb/devices/$USB_ID/$USB_ID:1.1/modalias; then
