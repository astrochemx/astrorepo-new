...
.SYNOPSIS
  ...
  Outputs optimized files into a sibling folder named "<input_folder_name>_OXMax", preserving subfolder structure.

DESCRIPTION
  - For each input path (folder or file) it will create a sibling folder named:
      "<input_folder_name>_OXMax"
...
USAGE
  ...
  - Or run manually:
      powershell -NoProfile -ExecutionPolicy Bypass -File .\optimize_pngs_max.ps1 "C:\path\to\folder" 6
...
if (-not $Paths -or $Paths.Count -eq 0) {
    ...
    Write-Host "  Drag folder(s) onto optimize_pngs_max.bat (uses default level 6)"
    Write-Host "  powershell -NoProfile -ExecutionPolicy Bypass -File .\optimize_pngs_max.ps1 `"$PWD\my_pngs`" 6"
    exit 1
}
...
function Optimize-To-Out([System.IO.FileInfo]$inputFile, [string]$outFilePath) {
    ...
    # Build arguments: --out <out> --opt <level> --preserve --strip safe --alpha --interlace 0 --scale16 --zopfli <in>
    $args = @('--out', $outFilePath, '--opt', $level.ToString(), '--preserve', '--strip', 'safe', '--alpha', '--interlace', '0', '--scale16', '--zopfli', $inputFile.FullName)
...
foreach ($raw in $Paths) {
    ...
    if ($item.PSIsContainer) {
        # Input is a folder: create sibling output folder named <input_folder_name>_OXMax
        ...
        $outputRoot = [System.IO.Path]::Combine($parent, "${baseName}_OXMax")
        ...
        if ($input -match '\.png$' -or $input -match '\.PNG$') {
            ...
            $baseName = [string]$baseName
            $outputRoot = [System.IO.Path]::Combine($fileParent, "${baseName}_OXMax")
