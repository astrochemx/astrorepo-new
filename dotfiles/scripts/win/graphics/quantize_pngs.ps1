<#
.SYNOPSIS
  Recursively optimize (losslessly) PNGs using pngquant.
  Outputs optimized files into a sibling folder named "<input_folder_name>_QUANTed", preserving subfolder structure.

DESCRIPTION
  - For each input path (folder or file) it will create a sibling folder named:
      "<input_folder_name>_QUANTed"
    and place PNGs inside, preserving subfolder structure.
  - Default speed level is 1 unless a numeric level is provided as the final argument.

USAGE
  - Drag-and-drop one or more folder(s) or file(s) onto a .bat wrapper that calls this script.
  - Or run manually:
      powershell -NoProfile -ExecutionPolicy Bypass -File .\quantize_pngs.ps1 "C:\path\to\folder" 1
    (last numeric argument 1-11 is treated as speed level if present; default = 1)
#>
...
function Find-PNGQuant {
    # Try PATH first:
    $cmd = Get-Command pngquant -ErrorAction SilentlyContinue
    if ($cmd) { return $cmd.Path }

    $scriptDir = Get-ScriptDir
    # Common install locations:
    $candidates = @(
		[System.IO.Path]::Combine($scriptDir, 'pngquant.exe'),
        [System.IO.Path]::Combine($scriptDir, 'bin', 'pngquant.exe'),
        [System.IO.Path]::Combine($scriptDir, 'pngquant', 'pngquant.exe'),
        [System.IO.Path]::Combine($scriptDir, 'pngquant', 'bin', 'pngquant.exe'),
        "$env:ProgramFiles(x86)\pngquant\bin\pngquant.exe",
        "$env:ProgramFiles(x86)\pngquant\pngquant.exe",
        "$env:ProgramFiles\pngquant\bin\pngquant.exe",
        "$env:ProgramFiles\pngquant\pngquant.exe",
        "$env:ProgramData\chocolatey\bin\pngquant.exe",
        "$env:USERPROFILE\.cargo\bin\pngquant.exe",
        "$env:USERPROFILE\scoop\shims\pngquant.exe"
    )
...
# If last argument is a digit 1-11, treat it as speed level and remove from Paths
if ($Paths -and ($Paths[-1] -match '^[1-11]$')) {
    $level = [int]$Paths[-1]
    if ($Paths.Count -gt 1) {
...
if (-not $Paths -or $Paths.Count -eq 0) {
    ...
    Write-Host "  Drag folder(s) onto quantize_pngs.bat (uses default speed level 1)"
    Write-Host "  powershell -NoProfile -ExecutionPolicy Bypass -File .\quantize_pngs.ps1 `"$PWD\my_pngs`" 1"
    exit 1
}

$pqPath = Find-PNGQuant
if (-not $pqPath) {
    Write-Error "pngquant executable not found. Place pngquant.exe next to this script, or install/add it to PATH."
    exit 2
}

Write-Host "Using pngquant: $pqPath (speed level: $level)"
Write-Host ""
...
function Optimize-To-Out([System.IO.FileInfo]$inputFile, [string]$outFilePath) {
    ...
    # Build arguments: --force --verbose --skip-if-larger --strip --nofs --ordered --speed=<level> <in>
    $args = @('--force', '--verbose', '--skip-if-larger', '--strip', '--nofs', '--ordered', '--output', $outFilePath, '--speed', $level.ToString(), $inputFile.FullName)

    $result = Run-Process-Collect -exe $pqPath -arguments $args
    ...
    if ($result.ExitCode -ne 0) {
        Write-Warning "pngquant returned exit code $($result.ExitCode) for $($inputFile.FullName)"
        return $false
    }
...
foreach ($raw in $Paths) {
    ...
    if ($item.PSIsContainer) {
        # Input is a folder: create sibling output folder named <input_folder_name>_QUANTed
        ...
        $outputRoot = [System.IO.Path]::Combine($parent, "${baseName}_QUANTed")
        ...
        if ($input -match '\.png$' -or $input -match '\.PNG$') {
            ...
            $baseName = [string]$baseName
            $outputRoot = [System.IO.Path]::Combine($fileParent, "${baseName}_QUANTed")
